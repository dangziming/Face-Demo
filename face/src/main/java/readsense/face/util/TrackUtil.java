    ...
    public static void drawAnim(List<YMFace> faces, SurfaceView outputView, float scale_bit, int cameraId, String fps, boolean showPoint) {
        ...
        try {
            ...
            for (int i = 0; i < faces.size(); i++) {
                ...
                int size = DisplayUtil.dip2px(BaseApplication.getAppContext(), 2);
...
//                    canvas.drawBitmap(bitmap, x1 + rect_text.width() + 5, y1 - bitmap.getHeight(), null);
    ...
    public static void drawAnimLiveness(List<YMFace> faces, SurfaceView outputView, float scale_bit, int cameraId, String fps) {
        if (paint == null) {
            paint = new Paint();
            paint.setAntiAlias(true);
        }
...
//                    canvas.drawBitmap(bitmap, x1 + rect_text.width() + 5, y1 - bitmap.getHeight(), null);
                    ...
                    float[] headposes = ymFace.getHeadpose();
                    StringBuffer sb = new StringBuffer();
                    if (!(Math.abs(headposes[0]) <= 20 && Math.abs(headposes[1]) <= 20
                            && Math.abs(headposes[2]) <= 20)) {
                        sb.append("请正脸面对");
                    } else {
                        paint.setColor(Color.RED);
                        if (ymFace.getFaceQuality() < 92) {
                            sb.append("人脸质量差 " + ymFace.getFaceQuality());
                        } else {
                            // 人脸位置
                            if (ymFace.getRect()[0] < 0 || ymFace.getRect()[1] < 0) {
                                sb.append("人脸超出边界");
                            } else {
                                // 人脸大小
                                int lim_w = iw > ih ? ih : iw;
                                if (rect[2] > lim_w * 2 / 3) {
                                    sb.append("人脸过大，请稍稍远离屏幕");
                                } else if (rect[2] < lim_w / 5) {
                                    sb.append("人脸过小，请离近一点");
                                } else {
                                    // 活体检测
